...
class TestBookView(APITestCase):
    def setUp(self) -> None:
        ...
        self.book1 = baker.make(models.Book, author=self.user1, genre=random.choice(genres), publication_date=timezone.now())
        self.book2 = baker.make(models.Book, author=self.user2, genre=random.choice(genres), publication_date=timezone.now())
        ...
        self.book_data = {
            ...
            "year": 1984,
            "genre": random.choice([i.name for i in genres])
        }
